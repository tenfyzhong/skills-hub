...
locals {
  ...
  extra_service_nodes = flatten([
    for service_name in sort(keys(local.extra_services)) : [
      for i in range(local.extra_services[service_name].count) : {
        key           = "${service_name}-${i}"
        service_name  = service_name
        index         = i
        instance_type = try(local.extra_services[service_name].instance_type, local.extra_service_instance)
        private_ip    = "172.31.${100 + index(sort(keys(local.extra_services)), service_name)}.${i + 1}"
      }
    ]
  ])
...
resource "aws_instance" "extra_service" {
  for_each = { for node in local.extra_service_nodes : node.key => node }
  ...
  tags = {
    Name    = "${local.namespace}-${each.value.service_name}-${each.value.index}"
