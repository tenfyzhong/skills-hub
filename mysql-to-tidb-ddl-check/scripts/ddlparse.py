...
from i18n import diagnostic, message
...
def column(tokens):
    ...
    if len(parts) != 1 or i >= len(tokens) or tokens[i].kw not in TYPES:
        raise ValueError(message('Column type or definition outside parser coverage'))
    typ = tokens[i].kw
    ...
    while j < len(attrs):
        ...
        if t.kw in ('DEFAULT', 'COMMENT', 'COLLATE', 'CHARSET', 'CONSTRAINT'):
            if j + 1 >= len(attrs):
                raise ValueError(message('Missing column attribute value'))
        ...
        if t.kw in ('CHECK', 'AS') and (j + 1 >= len(attrs) or attrs[j + 1].value != '('):
            raise ValueError(message('Missing expression parentheses'))
        if t.kw == 'CHARACTER' and j + 1 < len(attrs) and attrs[j + 1].kw == 'SET':
            j += 3
            continue
        if t.kind == 'word' and t.kw not in allowed:
            raise ValueError(message('Column attribute outside parser coverage'))
        j += 1
...
def parse_table(statement, database, defaults):
    ...
    if i >= len(tokens) or tokens[i].value != '(':
        raise ValueError(message('This release parses CREATE TABLE with explicit columns only; LIKE/AS SELECT is not covered'))
    end = closing(tokens, i)
    ...
    for group in split_top(tokens[i + 1:end]):
        try:
                ...
                if kind == 'FOREIGN':
                    ...
                    while j < len(tail):
                        if tail[j].kw != 'ON' or j + 2 >= len(tail) or tail[j + 1].kw not in ('DELETE', 'UPDATE'):
                            table.unknown.append((tail[j:], message('Foreign key option outside parser coverage')))
                            break
                        j += 2
                        if tail[j].kw in ('RESTRICT', 'CASCADE'):
                            ...
                            table.unknown.append((tail[j:], message('Foreign key reference action outside parser coverage')))
                            break
                    ...
                    if tail:
                        ...
                        while j < len(tail):
                            if tail[j].kw == 'USING' and j + 1 < len(tail) and tail[j + 1].kw == 'BTREE':
                                ...
                                table.unknown.append((tail[j:], message('Index option outside parser coverage')))
                                break
            elif kind == 'CHECK':
                if len(group) < 2 or group[1].value != '(':
                    raise ValueError(message('Missing CHECK expression'))
                table.checks.append(original)
            else:
                col = column(group)
                if col.name in table.columns:
                    raise ValueError(message('Duplicate column name'))
            ...
            table.unknown.append((group, diagnostic(exc)))
    return table
