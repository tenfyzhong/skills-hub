...
| cloud_plan / provider / region | Cloud plan, provider, and region; missing values make product-dependent decisions conditional |
...
| IDX-001 | FULLTEXT index; 8.5 / Cloud | 8.5: high/confirmed capability difference; parsing does not imply an effective index. Cloud depends on plan, region, and availability; unknown conditions require confirmation | Verify full-text queries and tokenization requirements; ordinary indexes or LIKE are not equivalent substitutes | S1, S2, S3 |
...
| INPUT-005 | Missing Cloud plan/region, outdated sources, or conflicting documentation | Continue common checks and retain product-specific uncertainty rather than assuming equivalence with self-managed 8.5 |
